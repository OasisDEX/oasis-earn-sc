...
import { _lpsToQuoteToken, _priceAt } from "./libraries/helpers/PoolHelper.sol";
...
contract PositionManager is ERC721, PermitERC721, IPositionManager, Multicall, ReentrancyGuard {
     ...
     *          - Burn
     */
    function burn(BurnParams calldata params_) external override mayInteract(params_.pool, params_.tokenId) {
    ...
    function moveLiquidity(
        ...
        (vars.bucketLPs, vars.bucketCollateral, vars.bankruptcyTime, vars.bucketDeposit, ) = IPool(params_.pool)
            .bucketInfo(params_.fromIndex);
        ...
        (vars.lpbAmountFrom, vars.lpbAmountTo, ) = IPool(params_.pool).moveQuoteToken(
            vars.maxQuote,
            params_.fromIndex,
            params_.toIndex,
            params_.expiry
    ...
    function getLPs(uint256 tokenId_, uint256 index_) external view override returns (uint256) {
    ...
    function getPositionInfo(uint256 tokenId_, uint256 index_) external view override returns (uint256, uint256) {
        return (positions[tokenId_][index_].lps, positions[tokenId_][index_].depositTime);
    }
