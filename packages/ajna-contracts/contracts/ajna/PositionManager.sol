...
} from "./interfaces/position/IPositionManagerDerivedState.sol";
...
contract PositionManager is PermitERC721, IPositionManager, Multicall, ReentrancyGuard {
  ...
  function moveLiquidity(
    ...
    (vars.bucketLP, vars.bucketCollateral, vars.bankruptcyTime, vars.bucketDeposit, ) = IPool(pool_)
      .bucketInfo(fromIndex_);
  ...
  function getLP(uint256 tokenId_, uint256 index_) external view override returns (uint256) {
    ...
    return
      _bucketBankruptAfterDeposit(IPool(tokenInfo.pool), index_, position.depositTime)
        ? 0
        : position.lps;
  }
  ...
  function getPositionIndexesFiltered(
    ...
    for (uint256 i = 0; i < indexesLength; ) {
      if (
        !_bucketBankruptAfterDeposit(pool, indexes[i], tokenInfo.positions[indexes[i]].depositTime)
      ) {
  ...
  function isPositionBucketBankrupt(
    ...
    return
      _bucketBankruptAfterDeposit(
        IPool(tokenInfo.pool),
        index_,
        tokenInfo.positions[index_].depositTime
      );
  }
