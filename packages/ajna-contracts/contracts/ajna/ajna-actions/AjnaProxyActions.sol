...
} from "../interfaces/position/IPositionManagerOwnerActions.sol";
...
contract AjnaProxyActions {
  ...
  function mintNft(ERC20Pool pool) internal returns (uint256 tokenId) {
    IPositionManagerOwnerActions.MintParams memory mintParams = IPositionManagerOwnerActions
      .MintParams(address(this), address(pool), keccak256("ERC20_NON_SUBSET_HASH"));
  ...
  function redeemPosition(uint256 price, uint256 tokenId, address pool) internal {
    ...
    IPositionManagerOwnerActions.RedeemPositionsParams
      memory redeemParams = IPositionManagerOwnerActions.RedeemPositionsParams(
        tokenId,
        address(pool),
        indexes
      );
  ...
  function memorializeLiquidity(uint256 price, uint256 tokenId, ERC20Pool pool) internal {
    ...
    IPositionManagerOwnerActions.MemorializePositionsParams
      memory memorializeParams = IPositionManagerOwnerActions.MemorializePositionsParams(
        tokenId,
        indexes
      );
  ...
  function moveLiquidity(
    ...
    IPositionManagerOwnerActions.MoveLiquidityParams
      memory moveParams = IPositionManagerOwnerActions.MoveLiquidityParams(
        tokenId,
        pool,
        oldIndex,
        newIndex,
  ...
  function unstakeNftAndRedeem(uint256 tokenId, ERC20Pool pool, uint256 price, bool burn) public {
    ...
    if (burn) {
      IPositionManagerOwnerActions.BurnParams memory burnParams = IPositionManagerOwnerActions
        .BurnParams(tokenId, address(pool));
