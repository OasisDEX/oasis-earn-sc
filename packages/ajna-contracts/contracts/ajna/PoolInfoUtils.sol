...
contract PoolInfoUtils is Multicall {
    ...
    )
  {
    ...
    (, bondFactor_, , kickTime_, referencePrice_, neutralPrice_, debtToCollateral_, , , ) = IPool(
      ajnaPool_
    ...
    if (kickTime_ != 0) {
      ...
      isCollateralized_ = _isCollateralized(
        debtToCover_,
        collateral_,
        vars.lup,
        IPool(ajnaPool_).poolType()
      );
    ...
    returns (uint256 debt_, uint256 collateral_, uint256 t0Np_, uint256 thresholdPrice_)
  {
    ...
    thresholdPrice_ = collateral_ == 0
      ? 0
      : Maths.wmul(Maths.wdiv(debt_, collateral_), COLLATERALIZATION_FACTOR);
  }
    ...
    )
  {
    ...
    pendingInterestFactor_ = PoolCommons.pendingInterestFactor(
      interestRate,
      block.timestamp - inflatorUpdate
    );
  }
  ...
  function availableQuoteTokenAmount(address ajnaPool_) external view returns (uint256 amount_) {
    ...
    uint256 poolBalance = IERC20Token(pool.quoteTokenAddress()).balanceOf(ajnaPool_) *
  ...
  function lenderInterestMargin(
    address ajnaPool_
  ) external view returns (uint256 lenderInterestMargin_) {
    IPool pool = IPool(ajnaPool_);
  ...
  function lpToQuoteTokens(
    ...
    IPool pool = IPool(ajnaPool_);
    (uint256 bucketLP_, uint256 bucketCollateral, , uint256 bucketDeposit, ) = pool.bucketInfo(
      index_
    );
