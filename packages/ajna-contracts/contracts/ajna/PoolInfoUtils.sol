...
contract PoolInfoUtils {
        ...
        )
    {
        ...
        (uint256 poolInflatorSnapshot, uint256 lastInflatorSnapshotUpdate) = pool.inflatorInfo();
        ...
        )
    {
        ...
        (uint256 inflatorSnapshot, uint256 lastInflatorSnapshotUpdate) = pool.inflatorInfo();
        ...
        pendingInterestFactor_ = PoolCommons.pendingInterestFactor(
            interestRate,
            block.timestamp - lastInflatorSnapshotUpdate
        );
    }
    ...
    function momp(address ajnaPool_) external view returns (uint256) {
        IPool pool = IPool(ajnaPool_);
    ...
    function lpsToCollateral(
        ...
        IPool pool = IPool(ajnaPool_);
        (uint256 bucketLPs_, uint256 bucketCollateral, , uint256 bucketDeposit, ) = pool.bucketInfo(index_);
        collateralAmount_ = _lpsToCollateral(bucketCollateral, bucketLPs_, bucketDeposit, lps_, _priceAt(index_));
    }
}
