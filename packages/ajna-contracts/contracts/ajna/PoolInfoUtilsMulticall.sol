...
contract PoolInfoUtilsMulticall {
   ...
   */
  function poolRatesAndFeesMulticall(
    address ajnaPool_
  )
    external
    view
    returns (uint256 lenderInterestMargin, uint256 borrowFeeRate, uint256 depositFeeRate)
  {
   ...
   */
  function poolBalanceDetails(
    address ajnaPool_,
    uint256 index_,
    address quoteTokenAddress_,
    address collateralTokenAddress_,
    bool isNFT_
  ) external view returns (PoolBalanceDetails memory poolBalanceDetails_) {
    IPool pool = IPool(ajnaPool_);

    // pool debtInfo
    (
      poolBalanceDetails_.debt,
      poolBalanceDetails_.accruedDebt,
      poolBalanceDetails_.debtInAuction,
      poolBalanceDetails_.t0Debt2ToCollateral
    ) = pool.debtInfo();
    ...
    if (isNFT_) {
      ...
      poolBalanceDetails_.collateralTokenBalance =
        IERC721(collateralTokenAddress_).balanceOf(ajnaPool_) *
        10 ** 18;
    } else {
      // normalize token balance to WAD scale
