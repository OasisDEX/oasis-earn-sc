...
describe.only('AAVE V3 | Claim Rewards | E2E', async () => {
  ...
  beforeEach(async () => {
    ...
    rewardsControllerProxyActionsContract = system.AaveRewardsProxyActions
      .contract as AaveRewardsProxyActions
    ...
    await accountGuard
      ...
      .setWhitelist(rewardsControllerProxyActionsAddress, true)
  })
  ...
  it('should claim rewards through proxy actions', async () => {
    ...
    const calldata = rewardsControllerProxyActionsContract.interface.encodeFunctionData(
      'claimAllRewards',
      [rewardsControllerAddress, assets],
    )
    const rewardTokenBalanceBefore = await ERC20__factory.connect(
      rewardToken,
      impersonatedSigner,
    ...
    const rewardBalanceAfter = await ERC20__factory.connect(
      rewardToken,
      impersonatedSigner,
    ).balanceOf(IMPERSONATED_SPARK_OWNER)
