...
import { SupportedTokens, getTokenAddress, tokenAmountToWei } from '../../utils/tokens';
...
function getAavePool(enviroment: Enviroment): Pool {
    ...
    const AavePool = new Contract(aaveAddress, AAVEPoolAbi, enviroment.provider) as any as Pool;
    return AavePool;
}
export async function aaveSupply(enviroment: Enviroment, from: Address, onBehalfOf: Address, asset: SupportedTokens, amount: number) {
...
export async function setAssetAsCollateral(enviroment: Enviroment, from: Address, asset: SupportedTokens) {
    const AavePool = getAavePool(enviroment);
    const assetAddress = getTokenAddress(asset, enviroment.network);

    const txData = await AavePool.setUserUseReserveAsCollateral.populateTransaction(
        assetAddress,
        true
    );
...
export async function aaveBorrow(enviroment: Enviroment, debt: SupportedTokens, amount: number, rate: AAVEBorrowRate = AAVEBorrowRate.VARIABLE) {
    const AavePool = getAavePool(enviroment);
    ...
    return sendTxFromAddress(txData, await enviroment.walletSigner.getAddress(), enviroment.provider);
}
