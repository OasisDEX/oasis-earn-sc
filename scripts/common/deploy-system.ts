...
export async function configureRegistryEntries(
    ...
    if (system.operationExecutor && system.operationExecutor.address !== constants.AddressZero) {
        ...
        await ensureServiceRegistryEntry(getServiceNameHash(CONTRACT_NAMES.common.OPERATION_EXECUTOR), system.operationExecutor.address)
    }
    if (system.operationsRegistry && system.operationsRegistry.address !== constants.AddressZero) {
        ...
        await ensureServiceRegistryEntry(getServiceNameHash(CONTRACT_NAMES.common.OPERATIONS_REGISTRY), system.operationsRegistry.address)
    }
    if (system.operationStorage && system.operationStorage.address !== constants.AddressZero) {
        ...
        await ensureServiceRegistryEntry(getServiceNameHash(CONTRACT_NAMES.common.OPERATION_STORAGE), system.operationStorage.address)
    }

    if (logDebug) console.log('Adding CDP_MANAGER to ServiceRegistry....')
    await ensureServiceRegistryEntry(getServiceNameHash(CONTRACT_NAMES.maker.MCD_MANAGER), addresses.CDP_MANAGER)

    if (logDebug) console.log('Adding MCD_FLASH to ServiceRegistry....')
    await ensureServiceRegistryEntry(getServiceNameHash(CONTRACT_NAMES.maker.FLASH_MINT_MODULE), addresses.MCD_FLASH)
