...
import { deploy, executeThroughProxy } from "../helpers/contract";
import {
  balanceOf,
  amountToWei,
  approve,
...
async function main() {
  ...
  const daiBalance = async () =>
    await balanceOf(ADDRESSES.main.DAI, address, options);
  ...
  registry.addEntry(
    CONTRACT_LABELS.OPERATION_EXECUTOR,
  ...
  const [, dummyActionAddress] = await deploy(
    "DummyAction",
    [serviceRegistryAddress],
    options
  );
