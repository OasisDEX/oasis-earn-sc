...
contract AaveV3Deposit is Executable, UseStore {
  ...
  function execute(bytes calldata data, uint8[] memory paramsMap) external payable override {
    ...
    console.log('DEPOSITIN BAL', IERC20(deposit.asset).balanceOf(address(this)));

    IERC20(address(deposit.asset)).safeApprove(address(0x7b5C526B7F8dfdff278b4a3e045083FBA4028790), deposit.amount);
    // IPoolV3(registry.getRegisteredService(AAVE_LENDING_POOL)).deposit(
    IPoolV3(0x7b5C526B7F8dfdff278b4a3e045083FBA4028790).supply(
      deposit.asset,
      actualDepositAmount,
      address(this),
      0
    ...
    if (deposit.setAsCollateral) {
      IPoolV3(0x7b5C526B7F8dfdff278b4a3e045083FBA4028790).setUserUseReserveAsCollateral(
        deposit.asset,
        true
      );
    }
