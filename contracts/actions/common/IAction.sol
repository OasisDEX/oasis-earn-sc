...
abstract contract IAction {
  ...
  function push(bytes32 value) internal {
    OperationStorage(registry.getRegisteredService("OPERATION_STORAGE")).push(value);
  }

  function pull(uint256 param, uint256 paramMapping) internal view returns (uint256) {
    if (paramMapping > 0) {
      bytes32 value = OperationStorage(registry.getRegisteredService("OPERATION_STORAGE")).at(
        paramMapping - 1
      );
  ...
  function at(uint256 index) internal view returns (bytes32) {
    return OperationStorage(registry.getRegisteredService("OPERATION_STORAGE")).at(index);
  }
}
