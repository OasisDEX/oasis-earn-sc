...
contract Payback is ActionBase {
  ...
  function _payback(PaybackParams memory params) internal returns (bytes32) {
    ...
    if (own == address(this) || mcdManager.cdpCan(own, params.vaultId, address(this)) == 1) {
      // Joins DAI amount into the vat
      daiJoin_join(params.userAddress, params.daiJoin, urn, params.amount);
      ...
      daiJoin_join(params.userAddress, params.daiJoin, address(this), params.amount);
      // Paybacks debt to the CDP
  ...
  function _paybackAll(PaybackParams memory params) internal returns (bytes32) {
    IManager mcdManager = IManager(params.mcdManager);
  ...
  function _getWipeDart(WipeDartParams memory params) internal view returns (int256 dart) {
  ...
  function _getWipeAllWad(WipeWadParams memory params) internal view returns (uint256 wad) {
    // Gets actual rate from the vat
    (, uint256 rate, , , ) = IVat(params.vat).ilks(params.ilk);
    // Gets actual art value of the urn
    (, uint256 art) = IVat(params.vat).urns(params.ilk, params.urn);
