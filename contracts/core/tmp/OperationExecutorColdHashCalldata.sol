...
interface IOperationStorage {
  ...
  function pushAction(bytes memory) external;
...
contract OperationExecutorColdHashCalldata is IERC3156FlashBorrower, IFlashLoanRecipient {
  ...
  function executeOp(Call[] calldata calls) public payable {
    ...
    for (uint256 current = 0; current < calls.length; current++) {
      actions = abi.encodePacked(actions, calls[current].targetHash);
    }
    bytes32 opHash = keccak256(actions);
  ...
  function aggregate(Call[] memory calls) internal {
    ...
    //     bytes memory actions;
    //     for (uint256 current = 0; current < calls.length; current++) {
    //       // callsHashes[current] = calls[current].targetHash;
    //       actions = abi.encodePacked(actions, calls[current].targetHash);

    //     }
    // opStorage.pushAction(actions);
    for (uint256 current = 0; current < calls.length; current++) {
