...
contract ProxyPermission {
  ...
  function givePermission(FlashloanData memory flData, address _contractAddr) public {
    

    if (flData.isProxyFlashloan) {
      if(flData.isDPMProxy) {
        // DPM permission
        IAccountGuard(IAccountImplementation(address(this)).guard()).permit(_contractAddr, address(this), true);
      } else {
        // DSProxy permission
  ...
  function removePermission(FlashloanData memory flData, address _contractAddr) public {
    if (flData.isProxyFlashloan) {
      if(flData.isDPMProxy) {
        // DPM permission
        IAccountGuard(IAccountImplementation(address(this)).guard()).permit(_contractAddr, address(this), false);
      } else {
        // DSProxy permission
        address currAuthority = address(DSAuth(address(this)).authority());
        if (currAuthority == address(0)) {
