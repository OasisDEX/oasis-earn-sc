...
export class Action {
  ...
  getParamTypes(abi: any): ParamType[] {
    return new ethers.utils.Interface(abi).functions['parseInputs(bytes)'].outputs![0].components
  }
  ...
  encodeCalldata() {
    const encodedArgs = ethers.utils.defaultAbiCoder.encode([this.getParamsTuple()], [this.args])
