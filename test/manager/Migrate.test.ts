...
describe(`Operations | Maker | ${OPERATION_NAMES.maker.OPEN_AND_DRAW}`, async () => {
  ...
  before(async () => {
    ...
    factory = system.manager.factory
  })
  ...
  const testName = `should have new owner after migration`
  it(testName, async () => {
    ...
    const migrateAction = createAction(
      await registry.getEntryHash(CONTRACT_NAMES.manager.MIGRATE_VAULT),
      [calldataTypes.manager.MigrateMakerVaults],
      [
        {
          cdpIds: [vault.id],
        },
    ...
    const preVaultOwner = await cdpManagerContract.owns(vault.id)

    /* call factory directly for debugging */
    /*     const [__, txReceipt2] = await executeThroughProxy(
      system.common.userProxyAddress,
      {
        address: system.manager.factory.address,
        calldata: system.manager.factory.interface.encodeFunctionData('migrateMaker', [[vault.id]]),
      },
      signer,
    ) */
    ...
    console.log(` pre  migration vault owner : ${preVaultOwner}`)
    console.log(` post migration vault owner : ${vaultOwner}`)

    const ownerAddress = await signer.getAddress()
    const data = await factory.accounts(ownerAddress, '0')

    expect(preVaultOwner).to.not.be.equal(vaultOwner)
    expect(vaultOwner).to.not.be.equal(data.vault)
  })
...
console.log(`ds proxy ${system.common.userProxyAddress}`)
const cdpManagerContract = new ethers.Contract(
  ADDRESSES.main.maker.cdpManager,
  CDPManagerABI,
  provider,
