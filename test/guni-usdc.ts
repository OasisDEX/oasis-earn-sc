...
describe('Guni open using USDC init collateral with dummy Exchange', async () => {
  ...
  describe(`Open Guni Vault using USDC init collateral with FL`, async () => {
    ...
    const usdcBalance = new BigNumber(10000)
    const expectedCR = new BigNumber(1.05)
    const leveragedAmount = usdcBalance.div(expectedCR.minus(one))
    const flashLoanAmount = leveragedAmount.minus(usdcBalance)
    ...
    let usdcAmount = await system.guniViewInstance.getOtherTokenAmount(
      guniDaiUsdc,
      gUniResolver,
    ...
    usdcAmount = new BigNumber(usdcAmount.toString())
      ...
      const pullToken = createAction(
        ...
        [{
          asset: ADDRESSES.main.USDC,
          from: address,
          amount: amountToWei(usdcBalance, 6).toFixed(),
        }],
        [0, 0, 0]
      );
      ...
      const pullTokenFromFlashloan = createAction(
        utils.keccak256(utils.toUtf8Bytes(CONTRACT_LABELS.common.PULL_TOKEN)),
        ["tupple(address asset, address from, uint256 amount)"],
        [{
      ...
      await USDC.approve(system.dsProxyInstance.address, ensureWeiFormat(ALLOWANCE))
      ...
      const swapAction = createAction(
        ...
        [ {
          ...
          amount: usdcAmount.minus(amountToWei(usdcBalance)).toFixed(),
          receiveAtLeast: amountToWei(1, 6).toFixed(), // just a number :D
          withData: 0,
        }],
        []
      )
