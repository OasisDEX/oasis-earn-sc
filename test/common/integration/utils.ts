...
export function printAllERC20Transfers(
  { events = [] }: ContractReceipt,
  labels
) {
  ...
  let contractEvents = events.filter(
    (x) => x.topics[0] === EventHash.ERC20_TRANSFER
  );
  ...
  contractEvents = events.filter(
    (x) => x.topics[0] === EventHash.WETH_WITHDRAWAL
  );
  for (let i = 0; i < contractEvents.length; i++) {
    packedEvents.push({
      AmountAsNumber: new BigNumber(contractEvents[i].data, 16)
...
export async function resetNetworkToBlock(
