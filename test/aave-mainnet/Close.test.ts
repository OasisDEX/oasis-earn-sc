...
import { DeployedMainnetSystemInfo, deployMainnetSystem } from '../instantiateSystem'
...
describe(`Strategy | AAVE | Close Position`, async () => {
  ...
  describe('Should close position with real oneInch', () => {
    ...
    let userStEthReserveData: AAVEReserveData
    let userAccountData: AAVEAccountData
    let system: DeployedMainnetSystemInfo
    ...
    before(async function () {
      ...
      if (shouldRun1InchTests) {
        ...
        const { system: _system } = await deployMainnetSystem(config, false, false)
        system = _system
